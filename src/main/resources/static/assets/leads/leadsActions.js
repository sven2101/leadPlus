...
LeadsCtrl.prototype.saveLead = function () {
	...
	this.newLead.orderPositions = this.currentOrderPositions;
	var process = {
		lead: this.newLead,
		status: 'OPEN'
	};
...
LeadsCtrl.prototype.addProduct = function () {
	if (!isNaN(Number(this.currentProductId)) && Number(this.currentProductId) > 0) {
		var tempProduct = findElementById(this.productService.products, Number(this.currentProductId));
...
LeadsCtrl.prototype.sumOrderPositions = function () {
	var sum = 0;
	if (this.currentOrderPositions == null) {
		this.currentOrderPositions = [];
	}
	for (var i = 0; i < this.currentOrderPositions.length; i++) {
		var temp = this.currentOrderPositions[i];
