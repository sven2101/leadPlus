...
// or undefined, passing each field username and effective value (as returned
// by getFieldValue) to the callback. If the object has no corresponding
// Def, the callback will never be called.
exports.eachField = function(object, callback, context) {
    getFieldNames(object).forEach(function(username) {
        callback.call(this, username, getFieldValue(object, username));
    }, context);
};
...
exports.someField = function(object, callback, context) {
    return getFieldNames(object).some(function(username) {
        return callback.call(this, username, getFieldValue(object, username));
    }, context);
};
```
...
```js
var copy = {};
require("ast-types").eachField(node, function(username, value) {
...
types.visit(ast, {
    ...
    visitMemberExpression: function(path) {
        ...
        if (n.Identifier.check(node.object) &&
            node.object.username === "arguments" &&
            n.Identifier.check(node.property)) {
            assert.notStrictEqual(node.property.username, "callee");
        }
...
types.visit(ast, {
    ...
    visitFunction: function(path) {
        ...
        //
        // where `rest` is the username of the rest parameter, and `n` is a
...
path.parentPath.username === "elements"
...
// property by username:
path.parent.node[path.parentPath.username][path.username] === path.node
```
...
`.depth`, `.declares(username)`, `.lookup(username)`, and `.getBindings()`.
...
// arguments, as named by .build("username", "program") above.
...
assert.strictEqual(main.username, "main.js");
assert.strictEqual(main.program.body[0].params[0].username, "x");
// etc.
