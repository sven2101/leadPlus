...
<br style="font-size: 13px;"><p style="font-size: 13px;">deliveryDate: &lt;#if deliveryDate??&gt;${deliveryDate}&lt;#else&gt;deliveryDate&lt;/#if&gt;</p>
 ...
 INSERT INTO public.template(content, deactivated, description, name)
    VALUES ( ' <div>
 <div>Sehr geehrter Herr ${customer.firstname} ${customer.lastname} </div>
 <div>Sehr geehrter Herr ${customer.firstname} ${customer.lastname} </div>

 <#list orderPositions as orderPosition> Item: ${orderPosition.product.name} - ${orderPosition.id}</#list>
 </div>', false, 'Simpel Template', 'Simple');