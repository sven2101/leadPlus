...
```
```js
const Hacker = Liftoff({username:'hacker'});
```
...
Sets the username of the configuration file Liftoff will attempt to find.  Case-insensitive.
...
Set extensions to include when searching for a configuration file.  If an external module is needed to load a given extension (e.g. `.coffee`), the module username should be specified as the value for the key.
...
```
myapp --myappfile /Users/username/Myappfile.js --cwd /var/www/project1
myapp --myappfile /Users/username/Myappfile.js --cwd /var/www/project2
```
...
#### require(username, module)

Emitted when a module is pre-loaded.

```js
var Hacker = new Liftoff({username:'hacker'});
Hacker.on('require', function (username, module) {
  console.log('Requiring external module: '+username+'...');
  // automatically register coffee-script extensions
  if (username === 'coffee-script') {
    module.register();
  }
});
```

#### requireFail(username, err)

Emitted when a requested module cannot be preloaded.

```js
var Hacker = new Liftoff({username:'hacker'});
Hacker.on('requireFail', function (username, err) {
  console.log('Unable to load:', username, err);
});
```
