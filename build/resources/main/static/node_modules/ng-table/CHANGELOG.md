...
````html
<!-- snip -->
<div ng-repeat="(username, filter) in column.filter">
<!-- snip -->
````
... now becomes:
...
````html
 <!-- snip -->
 <div ng-repeat="(username, filter) in column.filter(this)">
 <!-- snip -->
````
- **filters:**
...
* $$username field on filter definitions is not supported.

Previously:

````html
<td filter="{'username': 'text', $$username: 'username'}"</td>
````
... now becomes:
...
````html
        <input type="text" username="{{column.filterName}}"
````
... now becomes:
...
* Multiple filters defined by the *same* filter definition will now render each input with a seperate username.
...
````html
<td filter="{ 'username': 'text', templateURL: 'path/to/textFilter.html'}"</td>
````

... now becomes:

````html
<td filter="{ 'username': 'path/to/textFilter.html'}"</td>
````
...
- rename filter.username to filter.$$username according to issue #196
